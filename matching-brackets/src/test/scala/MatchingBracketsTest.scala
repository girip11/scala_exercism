...
class MatchingBracketsTest extends FunSuite with Matchers {
  ...
  test("paired square brackets") {
    ...
    MatchingBracketsRecursion.isPaired("[]") should be(true)
  }
  ...
  test("unpaired brackets") {
    ...
    MatchingBracketsRecursion.isPaired("[[") should be(false)
  }
  ...
  test("math expression") {
    ...
    MatchingBracketsRecursion.isPaired("(((185 + 223.85) * 15) - 543)/2") should be(true)
  }
  ...
  test("complex latex expression") {
    MatchingBrackets.isPaired(
      ...
      true)
    MatchingBracketsRecursion.isPaired(
      """\left(\begin{array}{cc} \frac{1}{3} & x\ \mathrm{e}^{x} &... x^2 \end{array}\right)""") should be(
      true)
  }
}
